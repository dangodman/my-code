<template>
  <div class="login">
    ...
    <div class="login-wrapper">
      <div class="avatar">
        <img src="https://th.bing.com/th?id=OIP.r0OnuYkvsbqBrYk3kUT53AHaKX&w=211&h=295&c=8&rs=1&qlt=90&o=6&pid=3.1&rm=2"
          alt="avater" />
      </div>
      <van-form @submit="onSubmit">
        ...
        <div style="margin: 16px;">
          <van-button round block color="#123456" native-type="submit">
            注册
          </van-button>
        </div>
      </van-form>
...
const onSubmit = async () => {
  ...
  try {
    const res = await axios.post('/register', { username: state.username, password: state.password, nickname: state.nickname })
    console.log(res)
    if (res.code === '8000') {
      router.push('/login')
    }
